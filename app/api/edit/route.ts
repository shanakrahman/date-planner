...
Keep all existing stops unless asked to change them. Preserve all existing fields. The JSON schema for each stop is:
{
  ...
  "must_try": [{"text": "string", "image": "string"}] — For restaurants/bars/cafes: 3 items with specific 2-4 word image queries. For galleries/parks: 1 item. Each item has text description and image search query.
  ...
  "lng": number
    ...
    try {
      ...
      updated = parsed as Itinerary;

      // Normalize must_try to new { text, image } format
      updated.stops = updated.stops.map((stop: any) => {
